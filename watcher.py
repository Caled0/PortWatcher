...
def config():															# Function to process config file input
	...
	ebuf = []
	checks = ["RECIPIENTS", "SCANNAME", "HOSTSFILE", "MINRATE", "MAXRATE", "RETRIES"]
...
def compare():															# Function to compare nmap scans
	...
	# Match things in not in new (old hosts)
				...
				for thing in b:
					if g == 1:
						if "." in thing:
							...
							diffout.append(str(thing) + " CLOSED")
				...
				for thing in tempbuf1:
					if "." in thing:				
						...
						if item == thing:
							g = 1
				if g == 0:
					diffout.append(str(item) + " CLOSED")
					statics.numclosed = statics.numclosed + 1
					n = 1
		if n == 0:
	...
	while k < s:
		if k == 0:
			...
			file.write("If IP does not have either, device has port changes in previous and current. \n")
		ch = diffout[k].split(" ")
		if "." in ch[0]:
			file.write("\n")
			try:
				file.write('%-15s %-25s %-10s\n' % (ch[0], '', ch[1]))
			except:
				file.write('%-15s %-25s %-10s\n' % (ch[0], '', ""))
		else:
			file.write('%-15s %-25s %-10s\n' % (ch[0], ch[1], ch[2]))
...
def mail():																# Function to send output	
	...
	if statics.change == 0:
		...
		comm = 'mutt -e "set content_type=text/html" -s ' + '"' + statics.scanname + " Change" + '" ' + statics.recipients + " -a /var/log/watcher/" + statics.scanname + "/ports.jpg" + " -a /var/log/watcher/" + statics.scanname + "/hosts.jpg " + "-- < " + statics.difffilename
		os.system(comm)
	else:
		print "No change.."
	print "Done..."
